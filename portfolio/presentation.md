...
| 4:30-5:45 | 5. MLOps | Run ID, Dataset Version, Metric, Artifact를 Job 수명주기와 연결 | Kubeflow/MinIO Evidence |
...
| 8:15-9:20 | 8. 실패와 한계 | Flannel Policy, local-path DR, 대규모 GPU, SLURM 실물 연동을 명시 | Verification Matrix |
| 9:20-10:00 | 9. NAIS 기여 | Adapter 기반 SLURM/Federation, 중앙 Identity/Audit, 원격 DR로 확장 | 직무요건 매핑표 |
...
| 질문 | 답변 축 |
|---|---|
| 왜 Kubernetes와 SLURM 중 Kubernetes를 먼저 선택했나? | 현재 장비와 기존 HAMi/Prometheus를 보존하면서 검증 가능한 최소 경로를 만들기 위해서다. SiteAdapter 뒤에 SLURM을 분리했다. |
...
것입니다. 입사 후에는 실제 SLURM·대규모 GPU·원격 DR 환경에서 이 경계를 확장하고
검증하겠습니다.”
