...
async function refreshAll({ quiet = false } = {}) {
  ...
  const [jobsResult, resourcesResult] = await Promise.allSettled([api("/v1/jobs"), api("/v1/resources/summary")]);
  ...
  if (resourcesResult.status === "rejected" && !quiet) toast(`자원 현황: ${resourcesResult.reason.message}`, "error");
  renderAll();
...
function switchView(name) {
  if (!['overview', 'jobs', 'submit'].includes(name)) return;
  state.activeView = name;
  document.querySelectorAll(".view").forEach((view) => view.classList.toggle("active", view.id === `${name}-view`));
  document.querySelectorAll("[data-view]").forEach((button) => button.classList.toggle("active", button.dataset.view === name));
  const labels = { overview: ["WORKSPACE", "개요"], jobs: ["EXECUTIONS", "작업"], submit: ["NEW EXECUTION", "새 작업"] };
