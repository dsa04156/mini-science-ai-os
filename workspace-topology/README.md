...
The canonical source files under `services/science_os` are root-owned in this workspace, so the changed files are kept as an explicit image overlay. The runtime image is based on `0.3.1` and published as `0.3.13-compact-nodes`.
