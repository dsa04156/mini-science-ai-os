...
통합한다. 이는 공고의 핵심 업무와 직접 대응한다. 다만 실물 SLURM 클러스터,
대규모 GPU 환경, 원격 백업 복구, MLflow/W&B 운영 경험은 증명하지 않는다.
...
| 자원 스케줄링 및 HPC | Kueue Queue/Quota와 HAMi 배치; SLURM CLI PoC | `scripts/demo.sh`, `portfolio/tests/test_slurm_adapter.py` | VERIFIED + POC | SLURM PoC는 제품 경로 및 실물 Scheduler에 미연결 |
...
| MLOps 운영 | Kubeflow Pipelines 2.17, MySQL Metadata, MinIO Artifact | `docs/evidence/verification-matrix.md` | VERIFIED | MLflow/W&B 직접 운영 증거는 없음 |
| Prometheus/Grafana 모니터링 | PrometheusRule, ServiceMonitor, DCGM, Dashboard JSON | `monitoring/grafana/science-ai-overview.json`, `docs/evidence/verification-matrix.md` | VERIFIED/PARTIAL | Grafana UI Import는 미검증 |
...
- SLURM PoC를 실물 HPC 운영 경험이라고 표현하지 않는다.
- Manifest가 있다는 이유만으로 Flannel NetworkPolicy가 강제된다고 주장하지 않는다.
- `local-path` PVC를 고가용성 저장소나 재해복구 완료 상태라고 표현하지 않는다.

## 보완 우선순위

1. 실제 SLURM Test Cluster에서 `sinfo/sbatch/squeue/sacct/scancel` 통합 검증
2. 외부 Object Storage와 MySQL Backup을 사용한 격리 Restore 성공 증거
3. Cilium/Calico 환경의 Cross-Namespace NetworkPolicy Negative Test
4. Grafana Dashboard Import와 Alert 전달 경로 검증
5. 공개 Git 저장소에서 CI, Argo CD Sync, Issue/PR 이력 확보
